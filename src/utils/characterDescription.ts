...
export const generateCharacterDescription = (character: Character): string => {
  const { name, gender, height, mass, hair_color, skin_color, eye_color, birth_year } = character;
  ...
  return (
    `${name} is a ${genderText.toLowerCase()} being with ${hairText} hair and ${skinText} skin. ` +
